...
def main():
    ...
    print(f"--- Adding 'Personal Training' Transaction for Member ID {member_id_to_use} ---")
    ...
    for activity in activities:
        ...
        if (activity[0] == 'Personal Training' and
            ...
            activity[4] is None and # end_date is None for PT
            activity[5] == transaction_amount and
            activity[6] == expected_payment_method_or_sessions_str): # sessions
            print(f"\nSuccessfully verified: Found matching 'Personal Training' transaction (ID: {activity[7]})")
            print(f"  Details: Type='{activity[0]}', Name='{activity[1]}', PaymentDate='{activity[2]}', Start='{activity[3]}', EndDate='{activity[4]}', Amount='{activity[5]}', Sessions='{activity[6]}'")
            verified = True
            break # Found the transaction

    if not verified:
        print("\nVerification Failed: Did not find the exact 'Personal Training' transaction in member's activity.")
        print(f"Expected: Type='Personal Training', PaymentDate='{today_date_str}', Start='{today_date_str}', EndDate=None, Amount='{transaction_amount}', Sessions='{expected_payment_method_or_sessions_str}'")
