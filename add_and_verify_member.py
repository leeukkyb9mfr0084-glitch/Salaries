...
try:
    ...
    print(
        "Ensure that the script is run from the project root and PYTHONPATH is set correctly if needed."
    )
    print(
        "If reporter is not in the current directory, you might need to run: export PYTHONPATH=/path/to/your/project/root"
    )
    sys.exit(1)
...
def main():
    ...
    print(
        f"Attempting to add member: {member_name}, Phone: {member_phone}, Join Date: {join_date}"
    )

    # Add member
    add_success = database_manager.add_member_to_db(
        member_name, member_phone, join_date
    )
        ...
        if members:
            ...
            for member in members:
                ...
                if member[1] == member_name and member[2] == member_phone:
                    print(
                        f"Successfully verified: Member found - ID: {member[0]}, Name: {member[1]}, Phone: {member[2]}, Join Date: {member[3]}"
                    )
                    verified = True
                    break
            if not verified:
                print(
                    f"Verification failed: Member with phone '{member_phone}' found, but details do not match."
                )
                print(f"Found members: {members}")
        else:
            print(
                f"Verification failed: Member with phone '{member_phone}' not found after add operation."
            )

    else:
        print(
            f"Failed to add member '{member_name}'. It might be due to an existing phone number or other database error."
        )
        # Attempt to query anyway, in case the error was misleading
        print(
            f"Checking if member '{member_name}' with phone '{member_phone}' already exists..."
        )
        members = database_manager.get_all_members(phone_filter=member_phone)
        if members:
            for member in members:
                if member[1] == member_name and member[2] == member_phone:
                    print(
                        f"Member '{member_name}' (Phone: {member_phone}) was already present in the database."
                    )
                    break
        else:
            print(
                f"Member '{member_name}' (Phone: {member_phone}) was not found, confirming add operation failed and member does not exist."
            )
