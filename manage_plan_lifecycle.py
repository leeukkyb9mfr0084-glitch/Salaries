...
def main():
    ...
    if success:
        ...
        if added_plan and added_plan[1] == initial_plan_name and added_plan[2] == initial_duration and added_plan[3] == 1: # is_active is index 3 (1 for True)
            print(f"Verification successful: Plan details match. Name: {added_plan[1]}, Duration: {added_plan[2]}, Active: {bool(added_plan[3])}")
        else:
        ...
        if existing_plan:
            print(f"Note: A plan with name '{initial_plan_name}' and duration {initial_duration} days (ID: {existing_plan[0]}) already exists. Using this for next steps.")
            ...
            if not existing_plan[3]: # if is_active is False
                print(f"Activating existing plan ID {new_plan_id} for test consistency.")
    ...
    update_success = database_manager.update_plan(new_plan_id, initial_plan_name, updated_duration)

    if update_success:
        print(f"Success: Plan ID {new_plan_id} duration updated to {updated_duration} days.")
        # Verification
        all_plans = database_manager.get_all_plans_with_inactive()
        edited_plan = find_plan_in_list(all_plans, new_plan_id)
        if edited_plan and edited_plan[2] == updated_duration:
            print(f"Verification successful: Plan duration is now {edited_plan[2]} days.")
        else:
    ...
    if deactivate_success:
        ...
        if deactivated_plan_check and deactivated_plan_check[3] == 0: # Check for 0 (False)
            print(f"Verification successful: Plan ID {new_plan_id} is_active status is False.")
        else:
            print(f"Verification failed for plan deactivation. Found: {deactivated_plan_check}")
            return
    else:
    ...
    if not found_in_active:
        print(f"Success: Deactivated Plan ID {new_plan_id} ('{initial_plan_name}') is not found in the list of active plans.")
        print(f"Total active plans found: {len(active_plans)}")
    else:
        print(f"Error: Deactivated Plan ID {new_plan_id} was found in the list of active plans.")
