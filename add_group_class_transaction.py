...
def main():
    ...
    existing_plans_with_name = [
        p for p in plans if p[1] == plan_name and p[2] == plan_duration_days
    ]

    if existing_plans_with_name:
        target_plan_id = existing_plans_with_name[0][0]  # Use the first match
        print(
            f"Using existing active plan: ID {target_plan_id}, Name: {existing_plans_with_name[0][1]}, Duration: {existing_plans_with_name[0][2]} days."
        )
    else:
        print(
            f"No existing active plan named '{plan_name}' with {plan_duration_days} days duration found. Attempting to add new plan."
        )
        ...
        identical_inactive_plan = next(
            (
                p
                for p in all_plans_incl_inactive
                if p[1] == plan_name and p[2] == plan_duration_days and not p[3]
            ),
            None,
        )
        ...
        if identical_inactive_plan:
            ...
            print(
                f"Found identical inactive plan: ID {target_plan_id}. Activating and using it."
            )
            activated = database_manager.set_plan_active_status(target_plan_id, True)
            if not activated:
                print(
                    f"Error: Could not activate plan ID {target_plan_id}. Cannot proceed."
                )
                return
        else:
            print(
                f"Creating new plan: Name: {plan_name}, Duration: {plan_duration_days} days."
            )
            success, msg, target_plan_id = database_manager.add_plan(
                plan_name, plan_duration_days, is_active=True
            )
            if success:
                print(
                    msg
                )  # Should contain success message like "New plan created successfully: ID ..."
                ...
                print(
                    f"Error: Could not create new plan '{plan_name}'. Cannot proceed."
                )
                return
    ...
    if transaction_success:
        ...
        member_check = database_manager.get_all_members(
            phone_filter=None
    ...
    for activity in activities:
        ...
        if (
            ...
            and activity[1] == plan_name  # Plan name used as name_or_description
            and activity[2] == today_date_str  # payment_date
            and activity[3] == today_date_str  # start_date
            and activity[4] == expected_end_date_str  # end_date
            and activity[5] == transaction_amount
            and activity[6] == payment_method
        ):
            print(
                f"\nSuccessfully verified: Found matching 'Group Class' transaction (ID: {activity[7]})"
            )
            print(
                f"  Details: Type='{activity[0]}', Plan='{activity[1]}', PaymentDate='{activity[2]}', Start='{activity[3]}', End='{activity[4]}', Amount='{activity[5]}', Method='{activity[6]}'"
            )
            verified = True
    ...
    if not verified:
        ...
        print(
            f"Expected: Type='Group Class', Plan='{plan_name}', PaymentDate='{today_date_str}', Start='{today_date_str}', End='{expected_end_date_str}', Amount='{transaction_amount}', Method='{payment_method}'"
        )
