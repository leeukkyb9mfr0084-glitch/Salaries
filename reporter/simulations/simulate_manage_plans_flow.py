...
def main():
    ...
    print(f"Attempting to add valid plan: Name='{plan_name_part1}', Duration={duration_part1_str} days")

    try:
        success, message, returned_plans = controller.save_plan_action(
            plan_name=plan_name_part1, duration_str=duration_part1_str, plan_id_to_update=""
        )
        print(f"Controller action message: {message}")
        if success:
            print(f"SUCCESS (Part 1): controller.save_plan_action returned success.")
            added_plan_from_list = find_plan_in_list_by_name(returned_plans, plan_name_part1)
            if added_plan_from_list:
                test_plan_id = added_plan_from_list[0] # Get ID
                print(f"SUCCESS (Part 1): Verified plan in returned list. ID: {test_plan_id}, Details: {added_plan_from_list}")
                if not (added_plan_from_list[2] == int(duration_part1_str) and added_plan_from_list[3] == 1): # is_active should be True (1)
                    print(f"FAILURE (Part 1): Details mismatch in returned plan. Expected Duration={duration_part1_str}, Active=True.")
            else:
                ...
                if db_plan:
                    ...
                    print(f"INFO (Part 1): Found plan in DB direct query. ID: {test_plan_id}. This implies inconsistency with controller's return.")
                else:
    ...
    if test_plan_id:
        ...
        print(f"Attempting to edit Plan ID {test_plan_id}: New Name='{new_name_part2}', New Duration={new_duration_part2_str} days")

        try:
            success, message, returned_plans = controller.save_plan_action(
                plan_name=new_name_part2, duration_str=new_duration_part2_str, plan_id_to_update=str(test_plan_id)
            )
            print(f"Controller action message: {message}")
            if success:
                print(f"SUCCESS (Part 2): controller.save_plan_action (for update) returned success for Plan ID {test_plan_id}.")
                edited_plan = find_plan_in_list_by_id(returned_plans, test_plan_id)
                if (edited_plan and edited_plan[1] == new_name_part2 and edited_plan[2] == int(new_duration_part2_str)):
                    print(f"SUCCESS (Part 2): Verified plan update in returned list. Details: {edited_plan}")
                else:
                    print(f"FAILURE (Part 2): Verification failed for update in returned list. Found: {edited_plan}.")
            else:
                print(f"FAILURE (Part 2): controller.save_plan_action (for update) returned failure for Plan ID {test_plan_id}.")
        except Exception as e:
        ...
        try:
            ...
            if success:
                ...
                if deactivated_plan and deactivated_plan[3] == 0: # is_active should be False (0)
                    print(f"SUCCESS (Part 3): Verified plan deactivation in returned list. is_active={deactivated_plan[3]}. Details: {deactivated_plan}")
                else:
                    print(f"FAILURE (Part 3): Verification failed for deactivation in returned list. Found: {deactivated_plan}. Expected is_active=False.")
            else:
                print(f"FAILURE (Part 3): controller.toggle_plan_status_action returned failure for Plan ID {test_plan_id}.")
        except Exception as e:
    ...
    print(f"Attempting to add plan with empty name: Name='{empty_plan_name}', Duration={valid_duration_part4_str} days")
    try:
        success, message, _ = controller.save_plan_action(
            plan_name=empty_plan_name, duration_str=valid_duration_part4_str, plan_id_to_update=""
        )
        print(f"Controller action message: {message}")
        if not success:
            print("SUCCESS (Part 4): controller.save_plan_action correctly prevented adding plan with empty name.")
            if "cannot be empty" in message:
                 print("SUCCESS (Part 4): Correct error message received.")
            else:
                 print(f"WARNING (Part 4): Operation failed as expected, but message was: '{message}'")
        else:
            print("FAILURE (Part 4): controller.save_plan_action did NOT prevent adding plan with empty name.")
    except Exception as e:
    ...
    print(f"Attempting to add plan with non-positive duration: Name='{valid_plan_name_part5}', Duration={invalid_duration_part5_str} days")
    try:
        success, message, _ = controller.save_plan_action(
            plan_name=valid_plan_name_part5, duration_str=invalid_duration_part5_str, plan_id_to_update=""
        )
        print(f"Controller action message: {message}")
        if not success:
            print("SUCCESS (Part 5): controller.save_plan_action correctly prevented adding plan with non-positive duration.")
            if "must be a positive integer" in message:
                 print("SUCCESS (Part 5): Correct error message received.")
            else:
                 print(f"WARNING (Part 5): Operation failed as expected, but message was: '{message}'")
        else:
            print("FAILURE (Part 5): controller.save_plan_action did NOT prevent adding plan with non-positive duration.")
    except Exception as e:
