...
try:
    ...
    print(
        "Ensure that the script is run from a context where 'reporter' is discoverable."
    )
    print(
        "Make sure PYTHONPATH is set up correctly if running from outside the project root."
    )
    sys.exit(1)
...
def main_simulation_logic(controller: GuiController):  # controller is now passed in
    ...
    try:
        ...
        if success:
            ...
            if members:
                for member in members:
                    if member[1] == valid_name and member[2] == valid_phone:
                        print(
                            f"SUCCESS (Part 1): Verified member in DB - ID: {member[0]}, Name: {member[1]}, Phone: {member[2]}"
                        )
                        verified = True
                        break
            if not verified:
                print(
                    f"FAILURE (Part 1): Member with phone '{valid_phone}' not found in DB after add operation, or details mismatch."
                )
        else:
            print(
                f"FAILURE (Part 1): controller.save_member_action returned failure. Message: {message}"
            )
            ...
            if "already exist" in message:
                members = controller.db_manager.get_all_members(
                    phone_filter=valid_phone
                )
                if members and members[0][1] == valid_name:
                    print(
                        f"INFO (Part 1): Member '{valid_name}' with phone '{valid_phone}' was already in DB, as indicated by controller."
                    )
    ...
    print(
        f"Attempting to add member with empty name: Name='{empty_name}', Phone='{valid_phone_for_empty_test}'"
    )

    try:
        success, message = controller.save_member_action(
            empty_name, valid_phone_for_empty_test
        )
        print(f"Controller action message: {message}")
        ...
        if not success:
            ...
            if "cannot be empty" in message:  # Check for expected error message
                ...
                print(
                    "WARNING (Part 2): Incorrect error message received for empty name, but operation still failed as expected."
                )
        else:
            # This case should ideally not be reached if controller validation is correct
            print(
                "FAILURE (Part 2): controller.save_member_action did NOT prevent adding member with empty name."
            )
            # Verify if it was actually added to the DB (it shouldn't be)
            members = controller.db_manager.get_all_members(
                phone_filter=valid_phone_for_empty_test
            )
            if members:
                print(
                    f"FAILURE (Part 2): Member with empty name WAS FOUND in DB with phone '{valid_phone_for_empty_test}'."
                )
            else:
                print(
                    "INFO (Part 2): Member with empty name was not found in DB, but controller reported success, which is inconsistent."
                )

    except Exception as e:
        print(
            f"ERROR (Part 2): An exception occurred during invalid data simulation: {e}"
        )
...
if __name__ == "__main__":
    ...
    SIM_DB_FILE = os.path.join(
        SIM_DB_DIR, "simulation_kranos_data_add_member.db"
    )  # Specific DB
