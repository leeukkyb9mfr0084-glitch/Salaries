...
SIM_DB_FILE = os.path.join(
    SIM_DB_DIR, "simulation_kranos_data_add_gm.db"
)  # Specific DB for this sim


# --- Helper to find member by name or add ---
def get_member_by_name_or_add(
    controller: GuiController, name_to_find: str, new_phone_if_adding: str
) -> int | None:
    ...
    if members:
        for member in members:
            if member[1] == name_to_find:
                print(
                    f"Prerequisite: Found existing member '{name_to_find}' with ID {member[0]}."
                )
                return member[0]
    ...
    if success:
        members_after_add = controller.db_manager.get_all_members(
            phone_filter=new_phone_if_adding
        )
        if members_after_add:
            print(
                f"Prerequisite: Added and using new member '{name_to_find}' with ID {members_after_add[0][0]}."
            )
            return members_after_add[0][0]
    print(
        f"CRITICAL PREREQUISITE FAILURE: Could not find or add member '{name_to_find}'. Controller success: {success}, message: {message}"
    )
    return None  # Explicitly return None on failure


# --- Helper to find plan by name or add ---
def get_plan_by_name_or_add(
    controller: GuiController, name_to_find: str, duration_if_adding: int
) -> int | None:
    all_plans = controller.db_manager.get_all_plans_with_inactive()
    for (
        plan_data
    ) in all_plans:  # Renamed plan to plan_data to avoid conflict with date object
        if plan_data[1] == name_to_find:
            plan_id = plan_data[0]
            print(
                f"Prerequisite: Found existing plan '{name_to_find}' with ID {plan_id}."
            )
            if not plan_data[3]:  # if not is_active
                print(
                    f"Prerequisite: Plan '{name_to_find}' (ID {plan_id}) is inactive. Activating."
                )
                # Toggle status action returns (success, message, updated_plans_list)
                toggle_success, toggle_message, _ = (
                    controller.toggle_plan_status_action(
                        plan_id=plan_id, current_status=False
                    )
                )
                print(
                    f"Prerequisite controller (activate plan) message: {toggle_message}"
                )
                if not toggle_success:
                    print(
                        f"CRITICAL PREREQUISITE FAILURE: Could not activate plan '{name_to_find}'."
                    )
                    return None
            return plan_id
    ...
    add_success, add_message, _ = controller.save_plan_action(
        plan_name=name_to_find,
        duration_str=str(duration_if_adding),
        plan_id_to_update="",
    )
    ...
    if add_success:
        ...
        for new_plan_data in plans_after_add:
            if new_plan_data[1] == name_to_find:
                print(
                    f"Prerequisite: Added and using new plan '{name_to_find}' with ID {new_plan_data[0]}."
                )
                return new_plan_data[0]
    print(
        f"CRITICAL PREREQUISITE FAILURE: Could not find or add plan '{name_to_find}'. Controller success: {add_success}, message: {add_message}"
    )
    return None
...
def main_simulation_logic(controller: GuiController):  # controller is now passed in
    ...
    member_id_for_test = get_member_by_name_or_add(
        controller, "UI Test User Valid GM", gm_test_member_phone
    )
    ...
    print(
        f"Attempting to add valid GM: MemberID={member_id_for_test}, PlanID={plan_id_for_test}, PayDate='{today_str}', StartDate='{today_str}', Amount='{valid_amount_str}', Method='{valid_payment_method}'"
    )
    ...
    try:
        ...
        if success:
            ...
            activities = controller.db_manager.get_all_activity_for_member(
                member_id_for_test
            )
            verified = False
            if activities:
                for activity in activities:
                    if (
                        ...
                        and activity[2] == today_str
                        and activity[3] == today_str
                        and activity[4] == expected_end_date_str
                        and activity[5] == float(valid_amount_str)  # Compare float
                        and activity[6] == valid_payment_method
                    ):
                        # Assuming activity[1] (plan name) is correctly populated by get_all_activity_for_member
                        print(
                            f"SUCCESS (Part 1): Verified GM in member activity - TransID: {activity[7]}, Plan Name: {activity[1]}, Details: {activity}"
                        )
                        verified = True
                        break
            if not verified:
                print(
                    f"FAILURE (Part 1): GM transaction not found or details mismatch for MemberID {member_id_for_test}."
                )
                print(
                    f"Expected end date: {expected_end_date_str}. Activities found: {activities}"
                )
        else:
            print(
                f"FAILURE (Part 1): controller.save_membership_action returned failure."
            )
    ...
    try:
        ...
        if not success:
            ...
            if (
                "cannot be negative" in message
                or "must be a positive number" in message
            ):  # Adjust if controller message is different
                print("SUCCESS (Part 2): Correct error message received.")
            else:
                print(
                    f"WARNING (Part 2): Operation failed as expected, but message was: '{message}'"
                )
        else:
            print(
                "FAILURE (Part 2): controller.save_membership_action did NOT prevent GM with non-positive amount."
            )
    except Exception as e:
