...
def run_simulation(controller: GuiController):  # Accept controller as argument
    ...
    assert (
        member_details
    ), f"Could not retrieve test member '{TEST_MEMBER_PHONE}' after adding."
    member_id, _ = member_details
    print(f"Test member ID: {member_id}")

    active_plans = (
        controller.get_active_plans()
    )  # This uses controller's db_manager implicitly
    ...
    assert (
        db_setup_success
    ), f"Failed to ensure book status is 'open' for {TEST_MONTH_KEY} at setup."
    print(
        f"Ensured book status for {TEST_MONTH_KEY} is 'open' before first transaction."
    )
    ...
    assert (
        "closed" in status_msg.lower()
    ), f"Books for {TEST_MONTH_KEY} should be 'closed', but status is: {status_msg}"
    ...
    print(
        f"Attempt to add to closed month - Success: {success}, Message: {msg}"
    )  # More detailed print
    assert (
        success is False
    ), f"Test: Should fail to add transaction to closed month. Success was {success}, Message: {msg}"
    expected_fail_msg_part = f"books for {TEST_MONTH_KEY} are closed"
    assert (
        expected_fail_msg_part in msg.lower()
    ), f"Failure message '{msg}' did not contain expected part '{expected_fail_msg_part}'"
    ...
    print(f"Status check: {status_msg}")
    assert (
        "open" in status_msg.lower()
    ), f"Books for {TEST_MONTH_KEY} should be 'open', but status is: {status_msg}"
