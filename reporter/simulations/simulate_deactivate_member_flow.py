...
SIMULATION_DB_FILE = os.path.join(
    SIMULATION_DB_DIR, "simulation_kranos_data_del_member.db"
)
...
def main_simulation_logic(controller: GuiController):  # Renamed and controller passed
    ...
    success_add, _ = controller.db_manager.add_member_to_db(
        member_name, member_phone
    )  # Use controller.db_manager
    if not success_add:
        print(
            f"FAILURE: Could not add member '{member_name}' via controller.db_manager."
        )
        # cleanup_simulation_environment() # Cleanup is in __main__
        return

    members_before = controller.db_manager.get_all_members(
        phone_filter=member_phone
    )  # Use controller.db_manager
    if not members_before or members_before[0][1] != member_name:
        print(
            f"FAILURE: Member '{member_name}' not found in DB after add operation or details mismatch."
        )
        # cleanup_simulation_environment() # Cleanup is in __main__
        return
    ...
    if not plans:
        print(
            "FAILURE: No plans found in the database to create a transaction. Seeding might have failed."
        )
    ...
    if not tx_success:
        print(
            f"FAILURE: Could not add transaction for member ID {member_id_to_delete}."
        )
        # cleanup_simulation_environment() # Cleanup is in __main__
        return
    ...
    cursor_setup_check.execute(
        "SELECT COUNT(*) FROM transactions WHERE member_id = ?", (member_id_to_delete,)
    )
    ...
    if transactions_count_before == 0:
        print(
            f"FAILURE: Transaction for member ID {member_id_to_delete} not found before deactivation attempt."
        )
        # cleanup_simulation_environment() # Cleanup is in __main__
        return
    print(
        f"Found {transactions_count_before} transaction(s) for the member before deactivation."
    )
    ...
    deactivate_success, deactivate_message = controller.deactivate_member_action(
        member_id_to_delete
    )
    print(
        f"Controller action message: '{deactivate_message}' (Success: {deactivate_success})"
    )

    if not deactivate_success:
        print(
            f"FAILURE: controller.deactivate_member_action reported failure for member ID {member_id_to_delete}."
        )
    ...
    members_after = controller.db_manager.get_all_members(
        phone_filter=member_phone
    )  # Use controller.db_manager
    if not members_after:
        print(
            f"SUCCESS: Member '{member_name}' (ID: {member_id_to_delete}) correctly marked as inactive (not found by get_all_members)."
        )
    else:
        print(
            f"FAILURE: Member '{member_name}' (ID: {member_id_to_delete}) still found by get_all_members after deactivation attempt."
        )
    ...
    cursor_check.execute(
        "SELECT * FROM transactions WHERE member_id = ?", (member_id_to_delete,)
    )
    ...
    if len(transactions_after) == transactions_count_before:
        print(
            f"SUCCESS: Transactions for member ID {member_id_to_delete} correctly persisted after deactivation ({len(transactions_after)} found)."
        )
    else:
        print(
            f"FAILURE: Expected {transactions_count_before} transaction(s) for member ID {member_id_to_delete} to be preserved, but found {len(transactions_after)}."
        )
