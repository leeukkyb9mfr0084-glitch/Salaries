        ...
        for row in reader:
            ...
            try:
                ...
                if phone in processed_members:
                    ...
                    try:
                        ...
                        if member_id:
                            ...
                            logging.warning(f"Failed to add member (add_member returned None) for row {line_count}: {name}, {phone}")
                        ...
                        if existing_member_row:
                            ...
                            processed_members[phone] = member_id
                            logging.info(f"Found existing member by phone for row {line_count}: {name}, {phone}, ID: {member_id}")
                        else:
                            logging.error(f"Member add failed (ValueError) and fetch failed for row {line_count}: {name}, {phone}")
                            failed_rows.append((line_count, row, "Member add failed (ValueError) and fetch failed"))
                            continue

                if not member_id:
                    logging.warning(f"Could not obtain member_id for row {line_count}: {name}, {phone}")
                ...
                try:
                    ...
                    logging.warning(f"Could not parse plan duration for row {line_count}. Value was: '{plan_duration_str}'. Skipping row.")
                    failed_rows.append((line_count, row, f"Invalid Plan Duration: {plan_duration_str}"))
                    continue

                amount_str = str(row.get(" Amount ")).strip().replace("₹", "").replace(",", "")
                try:
                    plan_price = float(amount_str)
                except (ValueError, TypeError):
                    logging.warning(f"Could not parse price for row {line_count}. Setting price to 0.0. Value was: '{row.get(' Amount ')}'")
                    plan_price = 0.0
                ...
                if not plan_id:
                    logging.warning(f"Could not find or create plan for row {line_count}. Plan: {plan_type}, Duration: {plan_duration}. Skipping row.")
                    failed_rows.append((line_count, row, f"Failed to find or create plan: {plan_type} / {plan_duration}"))
                    continue
                ...
                if not purchase_date: # If payment date is missing, use start date
                    ...
                    logging.info(f"Purchase date missing for row {line_count}, using start date: {start_date}")


                cursor = db_mngr.conn.cursor()
                # plan_duration_days is now plan_duration (parsed int from CSV)
                start_date_obj = datetime.strptime(start_date, "%Y-%m-%d").date()
                end_date_obj = start_date_obj + timedelta(days=plan_duration -1) # Use plan_duration from CSV
                ...
                logging.error(f"Exception processing GC row {line_count}: {e} - Data: {dict(row)}", exc_info=True)
                failed_rows.append((line_count, row, str(e)))

    logging.info(f"GC data migration: Processed {line_count} rows. Migrated/updated: {success_count}. Failed: {len(failed_rows)}.")
    if failed_rows:
        ...
        for row in reader:
            ...
            try:
                ...
                if phone in processed_members:
                    ...
                    try:
                        ...
                        if member_id:
                            ...
                            logging.warning(f"Failed to add member (add_member returned None) for PT row {line_count}: {name}, {phone}")
                            failed_rows.append((line_count, row, "Failed to add member (add_member returned None)"))
                            continue
                    except ValueError: # Member with phone likely exists
                        cursor = db_mngr.conn.cursor()
                        ...
                        if existing_member_row:
                            member_id = existing_member_row[0]
                            processed_members[phone] = member_id
                            logging.info(f"Found existing member by phone for PT row {line_count}: {name}, {phone}, ID: {member_id}")
                        else:
                            logging.error(f"Member add failed (ValueError) and fetch failed for PT row {line_count}: {name}, {phone}")
                            failed_rows.append((line_count, row, "Member add failed (ValueError) and fetch failed for PT"))
                            continue

                if not member_id:
                    logging.warning(f"Could not obtain member_id for PT row {line_count}: {name}, {phone}")
                ...
                try:
                    ...
                    logging.warning(f"Invalid session count for PT row {line_count}. Value: '{sessions_csv}'. Setting to 0.")

                if not purchase_date:
                    logging.warning(f"Invalid purchase date for PT row {line_count}. Value: '{purchase_date_csv}'. Skipping.")
                ...
                failed_rows.append((line_count, row, str(e)))
                logging.error(f"Exception processing PT row {line_count}: {e} - Data: {dict(row)}", exc_info=True)

    logging.info(f"PT data migration: Processed {line_count} rows. Migrated: {success_count}. Failed: {len(failed_rows)}.")
    if failed_rows:
...
def migrate_historical_data():
    ...
    logging.info(f"Summary: GC (Success: {total_gc_success}, Failed: {total_gc_failed}), PT (Success: {total_pt_success}, Failed: {total_pt_failed})")
