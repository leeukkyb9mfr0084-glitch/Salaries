...
def test_add_member_flow(api_db_fixture): # Use new fixture
    ...
    success_dup, message_dup = app_api.add_member(name="Test Member", phone="1234567890")

    assert success_dup is False
    # AppAPI.add_member calls db_mngr.add_member_to_db, which returns specific error
    assert "UNIQUE constraint failed: members.phone" in message_dup or "Phone number '1234567890' likely already exists" in message_dup
...
def test_plan_management_flow(api_db_fixture): # Use new fixture
    ...
    success_add, message_add, new_plan_id = app_api.add_plan(
        name="Test Plan", duration_days=30, price=DEFAULT_PRICE, type_text=DEFAULT_TYPE_TEXT
    )
    ...
    assert new_plan_entry[5] == 1  # is_active (e.g. index 5 if price, type_text are added before it)
    ...
    plan_after_deactivate = db_mngr.get_plan_by_id(new_plan_id) # get_plan_by_id returns Optional[tuple]
...
def test_add_membership_flow(api_db_fixture): # Use new fixture
    ...
    add_member_success, add_member_message = db_mngr.add_member_to_db("Membership User", "000111222")
    ...
    add_plan_success, _, db_plan_id_val = db_mngr.add_plan("Membership Plan", 30, DEFAULT_PRICE, DEFAULT_TYPE_TEXT)
    ...
    pt_transaction = next((act for act in member_activity_pt if act[0] == "Personal Training"), None)
...
def test_deactivate_member_action_flow(api_db_fixture): # Use new fixture
    ...
    add_mem_success, add_mem_message = db_mngr.add_member_to_db(member_name, member_phone)
    ...
    if not plans:
        add_plan_success, _, plan_id_for_tx_val = db_mngr.add_plan("Default Test Plan API", 30, DEFAULT_PRICE, DEFAULT_TYPE_TEXT)
    ...
    db_mngr.add_transaction(
        transaction_type='Group Class', member_id=member_id_to_deactivate, plan_id=plan_id_for_tx_val,
        payment_date="2024-01-01", start_date="2024-01-01", amount_paid=50.00, payment_method="Cash"
    )
    transactions_before_deactivation = db_mngr.get_all_activity_for_member(member_id_to_deactivate)
    ...
    active_members_after_deactivation = db_mngr.get_all_members(phone_filter=member_phone)
    ...
    transactions_after_deactivation = db_mngr.get_all_activity_for_member(member_id_to_deactivate)
...
def test_delete_transaction_action_flow(api_db_fixture): # Use new fixture
    ...
    plans = db_mngr.get_all_plans()
    if not plans:
        add_plan_success, _, plan_id_val = db_mngr.add_plan("Default Plan API", 30, DEFAULT_PRICE, DEFAULT_TYPE_TEXT)
...
def test_delete_plan_action_flow(api_db_fixture): # Use new fixture
    ...
    add_plan_s1, _, plan_id_unused_val = app_api.add_plan(plan_name_unused, 10, DEFAULT_PRICE, DEFAULT_TYPE_TEXT)
    ...
    add_plan_s2, _, plan_id_used_val = app_api.add_plan(plan_name_used, 40, DEFAULT_PRICE, DEFAULT_TYPE_TEXT)
...
def test_generate_custom_pending_renewals_action_flow(api_db_fixture): # Use new fixture
    app_api, db_mngr = api_db_fixture # Unpack

    add_mem_s, _ = db_mngr.add_member_to_db("Renewal User Future API", "RF002") # Changed phone
    assert add_mem_s
    members_rf1 = db_mngr.get_all_members(phone_filter="RF002")
    m_id_rf1 = members_rf1[0][0]
    client_name_rf1 = members_rf1[0][1]

    add_plan_s, _, plan_id_30day_val = app_api.add_plan("30 Day Plan Future API", 30, DEFAULT_PRICE, DEFAULT_TYPE_TEXT)
    ...
    expected_end_date_str = end_date_rf1_target.strftime('%Y-%m-%d') # This needs to match how add_transaction calculates it
...
def test_get_finance_report_data_flow(api_db_fixture): # Use new fixture, renamed test
    ...
    add_mem_s, _ = db_mngr.add_member_to_db("Finance User API1", "FX002") # Changed phone
    assert add_mem_s
    m_fx1_id = db_mngr.get_all_members(phone_filter="FX002")[0][0]

    add_plan_s, _, plan_fx_id_val = app_api.add_plan("Finance Plan API", 30, DEFAULT_PRICE, DEFAULT_TYPE_TEXT)
    ...
    transactions_for_month = app_api.get_transactions_for_month(report_year, report_month)
    assert len(transactions_for_month) == 2

    # Verify amounts if needed, e.g., by summing amounts from transactions_for_month
    sum_from_transactions = sum(t[3] for t in transactions_for_month) # amount_paid is at index 3
    ...
    assert no_transactions_revenue == 0.0 # Or None, depending on AppAPI implementation. DBManager returns 0.0. AppAPI returns float | None. It should be 0.0

    no_transactions_list = app_api.get_transactions_for_month(report_year, report_month + 2)
