...
def test_add_pt_membership_success(db_manager_pt: DatabaseManager):
    ...
    pt_membership_id = db_manager_pt.add_pt_membership(
        member_id, purchase_d, amount_p, sessions_p
    )
    ...
    new_cursor.execute(
        "SELECT member_id, purchase_date, amount_paid, sessions_total, sessions_remaining FROM pt_memberships WHERE id = ?",
        (pt_membership_id,),
    )
    ...
    assert (
        record[4] == sessions_p
    )  # sessions_remaining is set to sessions_total by add_pt_membership


def test_get_all_pt_memberships_for_view_empty(
    db_manager_pt: DatabaseManager,
):  # Renamed for clarity
    """Tests retrieving all PT memberships for view when none exist."""
    all_pt_memberships = (
        db_manager_pt.get_all_pt_memberships_for_view()
    )  # Using _for_view method
    assert isinstance(all_pt_memberships, list)
    assert len(all_pt_memberships) == 0


def test_get_all_pt_memberships_for_view_with_data(
    db_manager_pt: DatabaseManager,
):  # Renamed for clarity
    ...
    id1 = db_manager_pt.add_pt_membership(
        member_id, date.today().strftime("%Y-%m-%d"), 150.0, 10
    )
    id2 = db_manager_pt.add_pt_membership(
        member_id, date.today().strftime("%Y-%m-%d"), 280.0, 20
    )

    all_pt_memberships_view = (
        db_manager_pt.get_all_pt_memberships_for_view()
    )  # Using _for_view method
    ...
    membership1 = next(
        m for m in all_pt_memberships_view if m.membership_id == id2
    )  # Package 2 (amount 280)
    membership2 = next(
        m for m in all_pt_memberships_view if m.membership_id == id1
    )  # Package 1 (amount 150)
...
def test_delete_pt_membership_success(db_manager_pt: DatabaseManager):
    ...
    pt_membership_id = db_manager_pt.add_pt_membership(
        member_id, date.today().strftime("%Y-%m-%d"), 100.0, 5
    )
    assert pt_membership_id is not None
