...
def test_add_member_success(db_manager: DatabaseManager):
    member_id = db_manager.add_member(
        MEMBER_JOHN["name"], MEMBER_JOHN["phone"], MEMBER_JOHN["email"]
    )
    ...
    cursor.execute(
        "SELECT name, phone, email, join_date, is_active FROM members WHERE id = ?",
        (member_id,),
    )
...
def test_add_member_duplicate_phone(db_manager: DatabaseManager):
    ...
    with pytest.raises(
        ValueError, match=f"Phone number {MEMBER_JOHN['phone']} already exists."
    ):
        db_manager.add_member(
            "Another Name", MEMBER_JOHN["phone"], "another.email@example.com"
        )
...
def test_get_all_members_multiple(db_manager: DatabaseManager):
    ...
    db_manager.add_member(
        MEMBER_JANE["name"], MEMBER_JANE["phone"], MEMBER_JANE["email"]
    )
    db_manager.add_member(
        MEMBER_ALICE["name"], MEMBER_ALICE["phone"], MEMBER_ALICE["email"]
    )
...
def test_update_member_success_all_fields(db_manager: DatabaseManager):
    ...
    result = db_manager.update_member(
        member_id, updated_name, updated_phone, updated_email, updated_is_active
    )
    ...
    cursor = db_manager.conn.cursor()
    cursor.execute(
        "SELECT name, phone, email, is_active FROM members WHERE id = ?", (member_id,)
    )
    member_data = cursor.fetchone()
    assert member_data is not None
...
def test_update_member_phone_uniqueness_violation(db_manager: DatabaseManager):
    member1_id = db_manager.add_member(
        MEMBER_JOHN["name"], MEMBER_JOHN["phone"], MEMBER_JOHN["email"]
    )
    member2_id = db_manager.add_member(
        MEMBER_JANE["name"], MEMBER_JANE["phone"], MEMBER_JANE["email"]
    )
    ...
    with pytest.raises(
        ValueError,
        match=f"Phone number {MEMBER_JOHN['phone']} already exists for another member.",
    ):
