...
GROUP_PLAN_QUARTERLY = {
    "name": "Quarterly Silver",
    "duration_days": 90,
    "default_amount": 270.0,
}
GROUP_PLAN_ANNUAL = {
    "name": "Annual Bronze",
    "duration_days": 365,
...
def test_add_group_plan_success(db_manager: DatabaseManager):  # RENAMED
    plan_id = db_manager.add_group_plan(
        GROUP_PLAN_MONTHLY["name"],
        GROUP_PLAN_MONTHLY["duration_days"],
        GROUP_PLAN_MONTHLY["default_amount"],
    )  # RENAMED
    assert plan_id is not None
    assert isinstance(plan_id, int)

    expected_display_name = generate_expected_display_name(
        GROUP_PLAN_MONTHLY["name"], GROUP_PLAN_MONTHLY["duration_days"]
    )

    cursor = db_manager.conn.cursor()
    cursor.execute(
        "SELECT name, duration_days, default_amount, display_name, is_active FROM group_plans WHERE id = ?",
        (plan_id,),
    )  # RENAMED table
...
def test_add_group_plan_duplicate_display_name(db_manager: DatabaseManager):  # RENAMED
    ...
    with pytest.raises(
        ValueError, match=f"Display name '{expected_display_name}' already exists."
    ):  # Error message from DB manager might still say "Display name"
        db_manager.add_group_plan(
            GROUP_PLAN_MONTHLY["name"],
            GROUP_PLAN_MONTHLY["duration_days"],
            GROUP_PLAN_MONTHLY["default_amount"] + 50.0,
        )  # RENAMED
...
def test_get_all_group_plans_multiple(db_manager: DatabaseManager):  # RENAMED
    ...
    db_manager.add_group_plan(
        GROUP_PLAN_QUARTERLY["name"],
        GROUP_PLAN_QUARTERLY["duration_days"],
        GROUP_PLAN_QUARTERLY["default_amount"],
    )  # RENAMED
    db_manager.add_group_plan(
        GROUP_PLAN_ANNUAL["name"],
        GROUP_PLAN_ANNUAL["duration_days"],
        GROUP_PLAN_ANNUAL["default_amount"],
    )  # RENAMED

    plans = (
        db_manager.get_all_group_plans()
    )  # RENAMED - now returns List[GroupPlanView]
    assert len(plans) == 3
    ...
    expected_monthly_display_name = generate_expected_display_name(
        GROUP_PLAN_MONTHLY["name"], GROUP_PLAN_MONTHLY["duration_days"]
    )
...
def test_update_group_plan_success_all_fields(db_manager: DatabaseManager):  # RENAMED
    ...
    expected_new_display_name = generate_expected_display_name(
        updated_name, updated_duration
    )

    result = db_manager.update_group_plan(
        plan_id, updated_name, updated_duration, updated_amount, updated_is_active
    )  # RENAMED
    ...
    assert plan_data[4] == (
        0 if not updated_is_active else 1
    )  # DB stores boolean as 0 or 1


def test_update_group_plan_partial_fields_no_display_name_change(
    ...
    assert plan_id is not None
    original_display_name = generate_expected_display_name(
        GROUP_PLAN_QUARTERLY["name"], GROUP_PLAN_QUARTERLY["duration_days"]
    )

    updated_amount = 280.0
    result = db_manager.update_group_plan(
        plan_id, default_amount=updated_amount, is_active=False
    )  # RENAMED
    assert result is True
...
def test_update_group_plan_display_name_uniqueness_violation(
    db_manager: DatabaseManager,
):  # RENAMED
    plan1_id = db_manager.add_group_plan(
        GROUP_PLAN_MONTHLY["name"],
        GROUP_PLAN_MONTHLY["duration_days"],
        GROUP_PLAN_MONTHLY["default_amount"],
    )  # RENAMED
    plan2_id = db_manager.add_group_plan(
        GROUP_PLAN_QUARTERLY["name"],
        GROUP_PLAN_QUARTERLY["duration_days"],
        GROUP_PLAN_QUARTERLY["default_amount"],
    )  # RENAMED
    ...
    expected_colliding_display_name = generate_expected_display_name(
        GROUP_PLAN_MONTHLY["name"], GROUP_PLAN_MONTHLY["duration_days"]
    )
    with pytest.raises(
        ValueError,
        match=f"Display name '{expected_colliding_display_name}' already exists for another group_plan.",
    ):  # Adjusted error message
        db_manager.update_group_plan(
            plan2_id,
            name=GROUP_PLAN_MONTHLY["name"],
            duration_days=GROUP_PLAN_MONTHLY["duration_days"],
        )  # RENAMED


def test_update_group_plan_same_details_no_display_name_change(
    db_manager: DatabaseManager,
):  # RENAMED
    plan_id = db_manager.add_group_plan(
        GROUP_PLAN_MONTHLY["name"],
    ...
    cursor = db_manager.conn.cursor()
    cursor.execute(
        "SELECT display_name, default_amount FROM group_plans WHERE id = ?", (plan_id,)
    )  # RENAMED table
    plan_data = cursor.fetchone()
...
def test_delete_group_plan_success(db_manager: DatabaseManager):  # RENAMED
    ...
    cursor = db_manager.conn.cursor()
    cursor.execute(
        "SELECT * FROM group_plans WHERE id = ?", (plan_id,)
    )  # RENAMED table
