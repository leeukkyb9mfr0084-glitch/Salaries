...
def test_add_transaction_to_closed_month(book_closing_fixture):  # Use new fixture
    ...
    test_month = 3
    payment_date_str = (
        f"{test_year:04d}-{test_month:02d}-15"  # A date within the test month
    )
    month_key = f"{test_year:04d}-{test_month:02d}"
    ...
    ), f"Failed to add plan during test setup: {message_add_plan}"
...
def test_delete_transaction_from_closed_month(book_closing_fixture):  # Use new fixture
    ...
    success_add_member_del, _ = db_mngr.add_member_to_db(
        member_name_del, member_phone_del
    )
    ...
    ), "Failed to retrieve member_id for delete test setup"
    ...
    ), f"Failed to add plan for delete test setup: {message_add_plan_del}"
    assert (
        plan_id_del is not None
    ), "Test plan setup for delete test failed to return a valid plan_id"
    ...
    assert (
        add_tx_success is True
    ), "Failed to add initial transaction for deletion test."

    cursor.execute(
        "SELECT transaction_id FROM transactions WHERE member_id = ? AND payment_date = ?",
        (member_id, payment_date_str),
    )
    ...
    close_success = app_api.set_book_status(
        month_key, "closed"
    )  # Changed controller to app_api
    assert close_success is True

    # 4. Attempt to delete the transaction from the closed month using AppAPI
    delete_success, delete_message = app_api.delete_transaction(
        transaction_id
    )  # Changed controller to app_api
    ...
    assert (
        expected_error_part in delete_message
    ), f"Error message mismatch: {delete_message}"
    ...
    assert (
        cursor.fetchone()[0] == 1
    ), "Transaction should still exist as books are closed."
    ...
    delete_success_reopened, delete_message_reopened = app_api.delete_transaction(
        transaction_id
    )  # Changed controller to app_api
    ...
    assert (
        cursor.fetchone()[0] == 0
    ), "Transaction should have been deleted after re-opening books."
