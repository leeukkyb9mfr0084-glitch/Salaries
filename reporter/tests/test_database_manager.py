...
def test_add_member_successful(db_manager_fixture): # Use new fixture
    ...
    add_success, add_message = db_manager_fixture.add_member_to_db(name, phone) # Use db_manager_fixture
...
def test_add_member_duplicate_phone(db_manager_fixture): # Use new fixture
    ...
    add_success1, add_message1 = db_manager_fixture.add_member_to_db(name1, phone) # Use db_manager_fixture
    ...
    add_success2, add_message2 = db_manager_fixture.add_member_to_db(name2, phone) # Use db_manager_fixture
...
def test_get_all_members_filter_by_name(db_manager_fixture): # Use new fixture
    ...
    # Filter by full name
    members = db_manager_fixture.get_all_members(name_filter="John Doe") # Use db_manager_fixture
    assert len(members) == 1
    assert members[0][1] == "John Doe"

    # Filter by partial name (case-insensitive)
    members = db_manager_fixture.get_all_members(name_filter="john") # Use db_manager_fixture
    assert len(members) == 2
    member_names = sorted([m[1] for m in members])
    assert member_names == ["John Doe", "John Smith"]

    # Filter by a name not present
    members = db_manager_fixture.get_all_members(name_filter="NonExistent") # Use db_manager_fixture
...
def test_get_all_members_filter_by_phone(db_manager_fixture): # Use new fixture
    ...
    members = db_manager_fixture.get_all_members(phone_filter="1234567890") # Use db_manager_fixture
    assert len(members) == 1
    assert members[0][2] == "1234567890"

    # Filter by partial phone number
    members = db_manager_fixture.get_all_members(phone_filter="123") # Use db_manager_fixture
    assert len(members) == 2
    member_phones = sorted([m[2] for m in members])
    assert member_phones == ["1230000000", "1234567890"]

    # Filter by a phone number not present
    members = db_manager_fixture.get_all_members(phone_filter="111") # Use db_manager_fixture
    assert len(members) == 0


def test_get_all_members_filter_by_name_and_phone(db_manager_fixture): # Use new fixture
    ...
    members = db_manager_fixture.get_all_members(name_filter="Alice Johnson", phone_filter="1112223333") # Use db_manager_fixture
    assert len(members) == 1
    assert members[0][1] == "Alice Johnson"
    assert members[0][2] == "1112223333"

    # Filter by partial name and partial phone
    members = db_manager_fixture.get_all_members(name_filter="Alice", phone_filter="111") # Use db_manager_fixture
    assert len(members) == 2
    ...
    members = db_manager_fixture.get_all_members(name_filter="Alice Johnson", phone_filter="000") # Use db_manager_fixture
    assert len(members) == 0

    # Filter by name and phone (phone matches, name doesn't)
    members = db_manager_fixture.get_all_members(name_filter="NonExistent", phone_filter="1112223333") # Use db_manager_fixture
    assert len(members) == 0
...
def test_get_all_plans_with_inactive_only_inactive(db_manager_fixture): # Use new fixture
    """Tests get_all_plans_with_inactive when only inactive plans are present."""
    cursor = db_manager_fixture.conn.cursor() # Use db_manager_fixture.conn
    cursor.execute("DELETE FROM plans")
    db_manager_fixture.conn.commit() # Use db_manager_fixture.conn

    add_success1, _, plan_id1 = db_manager_fixture.add_plan("Inactive Plan 1", 10, is_active=False) # Use db_manager_fixture
    add_success2, _, plan_id2 = db_manager_fixture.add_plan("Inactive Plan 2", 20, is_active=False) # Use db_manager_fixture
...
def test_get_all_plans_with_inactive_mixed(db_manager_fixture): # Use new fixture
    ...
    s1, _, _ = db_manager_fixture.add_plan("Active Plan Mix", 30, is_active=True) # Use db_manager_fixture
    s2, _, _ = db_manager_fixture.add_plan("Inactive Plan Mix", 60, is_active=False) # Use db_manager_fixture
    s3, _, _ = db_manager_fixture.add_plan("Another Active Mix", 90, is_active=True) # Use db_manager_fixture
...
def test_get_plan_by_name_and_duration_exists(db_manager_fixture): # Use new fixture
    ...
    plan = db_manager_fixture.get_plan_by_name_and_duration("Monthly - Unrestricted", 1) # Use db_manager_fixture
    assert plan is not None
    assert plan[1] == "Monthly - Unrestricted"
    assert plan[2] == 30
    assert plan[3] == 1

def test_get_plan_by_name_and_duration_not_exists(db_manager_fixture): # Use new fixture
    """Tests get_plan_by_name_and_duration for a non-existent plan."""
    plan = db_manager_fixture.get_plan_by_name_and_duration("NonExistent Plan XYZ", 10) # Use db_manager_fixture
    assert plan is None

def test_get_plan_by_name_and_duration_wrong_duration(db_manager_fixture): # Use new fixture
    """Tests get_plan_by_name_and_duration for an existing name but wrong duration."""
    plan = db_manager_fixture.get_plan_by_name_and_duration("Monthly - Unrestricted", 2) # Use db_manager_fixture
    assert plan is None
...
def test_get_or_create_plan_id_get_existing(db_manager_fixture): # Use new fixture
    ...
    retrieved_id = db_manager_fixture.get_or_create_plan_id(existing_plan_name, existing_duration_days) # Use db_manager_fixture
...
def test_get_or_create_plan_id_create_new(db_manager_fixture): # Use new fixture
    ...
    new_plan_id = db_manager_fixture.get_or_create_plan_id(new_plan_name, new_plan_duration) # Use db_manager_fixture
...
def test_add_transaction_group_class(db_manager_fixture): # Use new fixture
    ...
    add_mem_success, add_mem_message = db_manager_fixture.add_member_to_db(member_name, member_phone) # Use db_manager_fixture
...
def test_get_memberships_for_member_none(db_manager_fixture): # Use new fixture
    ...
    add_success, add_message = db_manager_fixture.add_member_to_db("Member C NoHistory", "3003003003") # Use db_manager_fixture
    ...
    history_c = db_manager_fixture.get_all_activity_for_member(member_c_id) # Use db_manager_fixture
    assert len(history_c) == 0, "Member C should have no activity history."


# --- New and Updated Tests ---

def test_add_transaction_group_class_invalid_plan_id(db_manager_fixture): # Use new fixture
...
def test_get_pending_renewals(db_manager_fixture): # Use new fixture
    ...
    for name, data in member_data.items():
        add_success, add_message = db_manager_fixture.add_member_to_db(name, data["phone"]) # Use db_manager_fixture
    ...
    for member_info, end_date_str, should_be_included in transactions_data:
        add_transaction_success, msg = db_manager_fixture.add_transaction( # Use db_manager_fixture
            transaction_type='Group Class',
    ...
    actual_renewals = db_manager_fixture.get_pending_renewals(target_year, target_month) # Use db_manager_fixture
...
def test_get_pending_renewals_none_when_no_relevant_data(db_manager_fixture): # Use new fixture
    ...
    add_mem_success, _ = db_manager_fixture.add_member_to_db("Test User No Renew", "TNR001") # Use db_manager_fixture
...
def test_get_finance_report(db_manager_fixture): # Use new fixture
    ...
    add_s1, _ = db_manager_fixture.add_member_to_db("Finance User1", "F001") # Use db_manager_fixture
    assert add_s1 is True
    add_s2, _ = db_manager_fixture.add_member_to_db("Finance User2", "F002") # Use db_manager_fixture
    assert add_s2 is True
    add_s3, _ = db_manager_fixture.add_member_to_db("Finance User3", "F003") # Use db_manager_fixture
...
def test_get_finance_report_no_transactions(db_manager_fixture): # Use new fixture
    ...
    total_revenue_future = db_manager_fixture.get_finance_report(future_year, future_month) # Use db_manager_fixture
    ...
    total_revenue_ancient = db_manager_fixture.get_finance_report(1900, 1) # Use db_manager_fixture
...
def test_get_finance_report_with_pt_bookings(db_manager_fixture): # Use new fixture
    ...
    add_s1, _ = db_manager_fixture.add_member_to_db("Finance User PT1", "FPT001") # Use db_manager_fixture
    assert add_s1 is True
    add_s2, _ = db_manager_fixture.add_member_to_db("Finance User PT2", "FPT002") # Use db_manager_fixture
    assert add_s2 is True
    ...
    actual_total_revenue = db_manager_fixture.get_finance_report(target_year, target_month) # Use db_manager_fixture
...
def test_join_date_standardization_new_member_then_group_membership(db_manager_fixture): # Use new fixture
    """Scenario 1: New member, then group membership. Join date becomes GM start date."""
    member_phone = "JD_GM01"
    add_mem_s, _ = db_manager_fixture.add_member_to_db("JoinDate User GM", member_phone) # Use db_manager_fixture
...
def test_join_date_standardization_new_member_then_pt_booking(db_manager_fixture): # Use new fixture
    """Scenario 2: New member, then PT booking. Join date becomes PT start date."""
    member_phone = "JD_PT01"
    add_mem_s, _ = db_manager_fixture.add_member_to_db("JoinDate User PT", member_phone) # Use db_manager_fixture
    assert add_mem_s is True
...
def test_join_date_standardization_existing_member_earlier_activity(db_manager_fixture): # Use new fixture
    """Scenario 3: Existing member, new activity earlier than current join_date."""
    member_phone = "JD_EARLY01"
    add_mem_s, _ = db_manager_fixture.add_member_to_db("JoinDate User Early", member_phone) # Use db_manager_fixture
    assert add_mem_s is True
    cursor = db_manager_fixture.conn.cursor() # Use db_manager_fixture.conn
    cursor.execute("SELECT member_id FROM members WHERE phone = ?", (member_phone,))
    member_id = cursor.fetchone()[0]
...
def test_join_date_standardization_existing_member_later_activity(db_manager_fixture): # Use new fixture
    """Scenario 4: Existing member, new activity later than current join_date."""
    member_phone = "JD_LATER01"
    add_mem_s, _ = db_manager_fixture.add_member_to_db("JoinDate User Later", member_phone) # Use db_manager_fixture
    assert add_mem_s is True
    cursor = db_manager_fixture.conn.cursor() # Use db_manager_fixture.conn
    cursor.execute("SELECT member_id FROM members WHERE phone = ?", (member_phone,))
    member_id = cursor.fetchone()[0]
...
def test_deactivate_member(db_manager_fixture): # Use new fixture
    ...
    add_mem_s, add_mem_msg = db_manager_fixture.add_member_to_db(member_name, member_phone) # Use db_manager_fixture
    ...
    success, message = db_manager_fixture.deactivate_member(member_id) # Use db_manager_fixture
...
def test_delete_transaction(db_manager_fixture): # Use new fixture
    ...
    db_manager_fixture.delete_transaction(transaction_id_to_delete) # Use db_manager_fixture
...
def test_delete_plan(db_manager_fixture): # Use new fixture
    ...
    add_s_unused, msg_unused_add, unused_plan_id_val = db_manager_fixture.add_plan(plan_name_unused, plan_duration_unused, is_active=True) # Use db_manager_fixture
    assert add_s_unused and unused_plan_id_val is not None, f"Failed to add unused plan for deletion test: {msg_unused_add}"

    result_unused, message_unused = db_manager_fixture.delete_plan(unused_plan_id_val) # Use db_manager_fixture
    ...
    add_s_used, msg_used_add, used_plan_id_val = db_manager_fixture.add_plan(plan_name_used, plan_duration_used, is_active=True) # Use db_manager_fixture
...
def setup_test_data_for_history_filters(db_manager_fixture_conn: sqlite3.Connection): # Use new fixture, pass conn
...
def test_get_transactions_with_member_details_no_filters(db_manager_fixture): # Use new fixture
    setup_test_data_for_history_filters(db_manager_fixture.conn) # Pass conn from fixture
    results = db_manager_fixture.get_transactions_with_member_details() # Use db_manager_fixture
    assert len(results) == 4, "Should return all 4 transactions"

def test_get_transactions_with_member_details_name_filter(db_manager_fixture): # Use new fixture
    setup_test_data_for_history_filters(db_manager_fixture.conn) # Pass conn
    results_alice = db_manager_fixture.get_transactions_with_member_details(name_filter="Alice") # Use db_manager_fixture
    ...
    results_bob = db_manager_fixture.get_transactions_with_member_details(name_filter="Bob The Builder") # Use db_manager_fixture
    assert len(results_bob) == 1, "Should return 1 transaction for 'Bob The Builder'"
    assert results_bob[0][10] == "Bob The Builder"

def test_get_transactions_with_member_details_phone_filter(db_manager_fixture): # Use new fixture
    setup_test_data_for_history_filters(db_manager_fixture.conn) # Pass conn
    results = db_manager_fixture.get_transactions_with_member_details(phone_filter="111-222-3333") # Use db_manager_fixture
    assert len(results) == 2, "Should return 2 transactions for Alice Wonderland's phone"
    assert results[0][11] == "111-222-3333"

    results_partial = db_manager_fixture.get_transactions_with_member_details(phone_filter="111") # Use db_manager_fixture
    assert len(results_partial) == 3, "Should return 3 transactions for phones starting with '111'"

def test_get_transactions_with_member_details_join_date_filter(db_manager_fixture): # Use new fixture
    setup_test_data_for_history_filters(db_manager_fixture.conn) # Pass conn
    results = db_manager_fixture.get_transactions_with_member_details(join_date_filter="2023-01-01") # Use db_manager_fixture
    assert len(results) == 3, "Should return 3 transactions for members joined on 2023-01-01"
    for record in results:
        assert record[12] == "2023-01-01"

def test_get_transactions_with_member_details_combined_filters(db_manager_fixture): # Use new fixture
    setup_test_data_for_history_filters(db_manager_fixture.conn) # Pass conn
    results = db_manager_fixture.get_transactions_with_member_details(name_filter="Alice", phone_filter="111-444-5555") # Use db_manager_fixture
    ...
    results = db_manager_fixture.get_transactions_with_member_details(name_filter="Bob", join_date_filter="2023-02-15") # Use db_manager_fixture
    assert len(results) == 1, "Should return 1 transaction for Bob by name and join date"
    assert results[0][10] == "Bob The Builder"

def test_get_transactions_with_member_details_invalid_date_format_filter(db_manager_fixture): # Use new fixture
    setup_test_data_for_history_filters(db_manager_fixture.conn) # Pass conn
    results = db_manager_fixture.get_transactions_with_member_details(join_date_filter="01/01/2023") # Use db_manager_fixture
    assert len(results) == 4, "Should ignore invalid date format and return all transactions"

def test_get_transactions_with_member_details_no_results(db_manager_fixture): # Use new fixture
    setup_test_data_for_history_filters(db_manager_fixture.conn) # Pass conn
    results = db_manager_fixture.get_transactions_with_member_details(name_filter="NonExistentName") # Use db_manager_fixture
    assert len(results) == 0, "Should return 0 transactions for a non-existent name"

    results = db_manager_fixture.get_transactions_with_member_details(join_date_filter="1900-01-01") # Use db_manager_fixture
    assert len(results) == 0, "Should return 0 transactions for a non-existent join date"

    results = db_manager_fixture.get_transactions_with_member_details(name_filter="Alice", phone_filter="999-999-9999") # Use db_manager_fixture
...
def test_set_and_get_book_status_closed(db_manager_fixture): # Use new fixture
    ...
    month_key = "2023-11"
    set_success = db_manager_fixture.set_book_status(month_key, "closed") # Use db_manager_fixture
...
def test_set_and_get_book_status_open(db_manager_fixture): # Use new fixture
    ...
    month_key = "2023-10"
    set_success = db_manager_fixture.set_book_status(month_key, "open") # Use db_manager_fixture
...
def test_set_book_status_update_existing(db_manager_fixture): # Use new fixture
    ...
    month_key = "2023-09"
    set_initial_success = db_manager_fixture.set_book_status(month_key, "closed") # Use db_manager_fixture
    assert set_initial_success is True, "Initial set_book_status to 'closed' failed."
    set_update_success = db_manager_fixture.set_book_status(month_key, "open") # Use db_manager_fixture
...
def _setup_member_and_plan_for_transaction_tests(db_manager_fixture_param, member_phone_suffix="TRN"): # Renamed param to avoid conflict
...
def test_delete_transaction_when_books_closed(db_manager_fixture): # Use new fixture
    ...
    delete_success, message = db_manager_fixture.delete_transaction(transaction_id_to_delete) # Use db_manager_fixture
