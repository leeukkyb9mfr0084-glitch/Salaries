...
class AppAPI:
    ...
    def __init__(self) -> None:
        ...
        """
        conn = sqlite3.connect(DB_FILE, check_same_thread=False) # check_same_thread for web apps
        self.db_manager: DatabaseManager = DatabaseManager(connection=conn)

    # Member operations
    def add_member(self, name: str, email: str, phone: str, join_date: str) -> Optional[models.Member]:
        """
        Adds a new member.
        """
    ...
    def update_member(self, member_id: int, name: Optional[str] = None, phone: Optional[str] = None, email: Optional[str] = None, join_date: Optional[str] = None, is_active: Optional[bool] = None) -> bool:
        """
    ...
    def add_group_plan(self, name: str, duration_days: int, default_amount: float, is_active: bool = True) -> Optional[models.GroupPlan]:
        """
    ...
    def update_group_plan(self, plan_id: int, name: Optional[str] = None, duration_days: Optional[int] = None, default_amount: Optional[float] = None, display_name: Optional[str] = None, is_active: Optional[bool] = None) -> bool:
        """
    ...
    def get_group_plan_by_display_name(self, display_name: str) -> Optional[models.GroupPlanView]:
        ...
        if db_plan:
            return models.GroupPlanView(
                ...
                name=db_plan.name,
                display_name=db_plan.display_name if db_plan.display_name else f"{db_plan.name} - {db_plan.duration_days} days",
    ...
    def create_group_class_membership(self, member_id: int, plan_id: int, start_date: str, amount_paid: float, purchase_date: str, payment_method: Optional[str] = None, notes: Optional[str] = None) -> Optional[models.GroupClassMembership]:
        """
        ...
        try:
            ...
            duration = int(plan_details.duration_days) if plan_details and hasattr(plan_details, 'duration_days') and plan_details.duration_days is not None else 0
            # Ensure end_date calculation is correct: if duration is 30 days, it ends 29 days after start.
            end_date_obj = start_date_obj + timedelta(days=duration - 1 if duration > 0 else 0)
        ...
        existing_memberships = self.db_manager.get_group_class_memberships_by_member_id(member_id)
    ...
    def get_all_group_class_memberships_for_view(self, name_filter: Optional[str] = None, status_filter: Optional[str] = None) -> List[models.GroupClassMembershipView]:
        # Assumes db_manager.get_all_group_class_memberships_for_view returns List[models.GroupClassMembershipView]
        return self.db_manager.get_all_group_class_memberships_for_view(name_filter=name_filter, status_filter=status_filter)

    def update_group_class_membership(self, membership_id: int, member_id: int, plan_id: int, start_date: str, amount_paid: float, purchase_date: Optional[str]=None, membership_type: Optional[str]=None, is_active: Optional[bool]=None, payment_method: Optional[str] = None, notes: Optional[str] = None) -> bool:
        """
        ...
        membership_to_update = models.GroupClassMembership(
            ...
            membership_type=membership_type if membership_type is not None else "New", # Provide a default if None
            is_active=is_active if is_active is not None else True, # Provide a default if None
    ...
    def create_pt_membership(self, member_id: int, purchase_date: str, amount_paid: float, sessions_total: int) -> Optional[models.PTMembership]:
        """
    ...
    def update_pt_membership(self, membership_id: int, member_id: int, purchase_date: str, amount_paid: float, sessions_total: int, sessions_remaining: int) -> bool:
        """
    ...
    def generate_renewal_report(self) -> List[Dict[str, Any]]:
        ...
        thirty_days_later_obj = date.today() + timedelta(days=30) # original was 30, not 29
